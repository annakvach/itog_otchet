...
```{r}
my.files <-  list.files(list.dirs(path = "~/itog_otchet/salmon_quant_output", full.names = TRUE, recursive = FALSE), pattern = "quant.sf", full.names = TRUE)
```
...
```{r}
tab <- readDGE(files = my.files, columns = c(1, 5), group = c(1,1,1, 2,2,2, 3,3,3, 4,4,4))  
```
...
```{r}
AveLogCPM <- aveLogCPM(tab_filtered)
hist(AveLogCPM)
```
...
```{r}
sample_names = c("Bud_auto_29", "Bud_auto_39", "Bud_auto_40", "Bud_avic_23", "Bud_avic_24", "Bud_avic_25", "Mat_auto_26", "Mat_auto_27", "Mat_auto_28", "Mat_avic_22", "Mat_avic_37", "Mat_avic_38")

plotMDS(tab_filt_norm, col=rep(1:4, each=3), labels = sample_names)
```
...
```{r}
y <- estimateGLMCommonDisp(tab_filt_norm, design_each_sample)
```
