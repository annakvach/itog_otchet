...
fastqc -o /path_to/FastQC_1_for_raw_data * 
```
...
while read ID
do
        mv ${ID}_rna_removed.1 ${ID}_rna_removed_1_F.fastq.gz
        mv ${ID}_rna_removed.2 ${ID}_rna_removed_2_F.fastq.gz

done < $FILE

```

3) запустить этот блок
...
```{bash, eval=F, echo=T}
gzip Trinity_2019filtered_Reference.fasta
```

```{bash, eval=F, echo=T}

/home/tools/salmon-latest_linux_x86_64/bin salmon index -t Trinity_2019filtered_Reference.fasta.gz -i ref_index_1
